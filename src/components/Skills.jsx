...
function RollingNumber({ valueStr, visible, delay }) {
  const [count, setCount] = useState(0);
  const match = valueStr.match(/^(\d+)(.*)$/);
  const target = match ? parseInt(match[1], 10) : (valueStr === '∞' ? '∞' : 0);
  ...
  if (target === '∞') return <span className="">∞</span>;
  return <span className="">{count}{suffix}</span>;
}
...
export default function Skills({ dark }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      halo.addColorStop(0, dark ? 'rgba(0,168,255,0.09)' : 'rgba(0,200,130,0.07)');
      ...
      rim.addColorStop(0, dark ? 'rgba(0,168,255,0.55)' : 'rgba(0,190,120,0.4)');
      ...
      projected.forEach(({ sx, sy, z, scale, i }) => {
        ...
        if (isHov) {
          ...
          glow.addColorStop(0, 'rgba(0,168,255,0.4)');
        ...
        ctx.fill();
        ctx.strokeStyle = isHov ? 'rgba(0,168,255,1)' : dark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.1)';
        ...
        if (depth > 0.52) {
          ctx.fillStyle = isHov ? '#00a8ff' : dark ? 'rgba(255,255,255,0.72)' : 'rgba(0,0,0,0.65)';
  ...
  return (
    <section id="skills" ref={sectionRef} style={{ padding: '120px 2rem', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: 'linear-gradient(90deg,transparent,rgba(0,168,255,0.4),transparent)' }} />
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 700, height: 700, borderRadius: '50%', background: 'radial-gradient(circle,rgba(0,168,255,0.04) 0%,transparent 70%)', pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 'clamp(36px,5vw,56px)', fontWeight: 800, color: dark ? '#fff' : '#0a0a0f', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(40px) scale(0.9) skewY(2deg)', filter: visible ? "blur(0px)" : "blur(12px)", transition: 'all 0.9s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            Compétences <span style={{ color: '#00a8ff' }}>Techniques</span>
          </h2>
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, marginTop: 14 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginBottom: 48, flexWrap: 'wrap' }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} style={{
              ...
              border: `1px solid ${activeCategory === cat ? '#00a8ff' : dark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.1)'}`,
              background: activeCategory === cat ? 'rgba(0,168,255,0.08)' : 'transparent',
              color: activeCategory === cat ? '#00a8ff' : dark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)',
              boxShadow: activeCategory === cat ? '0 0 16px rgba(0,168,255,0.15)' : 'none',
            }}>
              {cat}
            </button>
          ))}
        ...
        {/* Stats */}
        <div style={{ display: 'flex', justifyContent: 'center', gap: '4rem', marginTop: 60, paddingTop: 40, borderTop: `1px solid ${dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`, flexWrap: 'wrap' }}>
          {[{ value: '12+', label: 'Projets Finis' }, { value: '10+', label: 'Langages maîtrisés' }, { value: '4+', label: 'Base de données' }, { value: '∞', label: 'Curiosité tech' }].map((s, i) => (
            <div key={s.label} style={{
              ...
              <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 900, fontSize: 32 }}>
                <RollingNumber valueStr={s.value} visible={visible} delay={0.4 + i * 0.15} />
              </div>
              <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, marginTop: 6, color: dark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)', textTransform: "uppercase", letterSpacing: "0.1em" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {tooltip && (
        <div style={{
          ...
          border: '1px solid rgba(0,168,255,0.45)', borderRadius: 10, padding: '8px 16px',
          fontFamily: "'Space Mono',monospace", pointerEvents: 'none', zIndex: 1000,
          boxShadow: '0 8px 30px rgba(0,168,255,0.2)', backdropFilter: 'blur(12px)',
          animation: 'tooltipIn 0.15s ease both',
        }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#00a8ff' }}>{tooltip.name}</div>
